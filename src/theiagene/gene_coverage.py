...
def quantify_gene_coverage(
    ...
    for contig, features in grouped_features.items():
        ...
        for feature in features:
            ...
            depths = []
            coverages = []
            # only extract the subfeatures of interest
            for subfeature in extract_features(feature.descendants, feature_type, exact_match):
...
def run_cli(args: argparse.Namespace) -> int:
    ...
    # convert GFF into features
    features_dict = assimilate_gff(args.reference_gff)
    ...
    if ambiguous_contig:
        ...
        features_dict = {imported_bam.references[0]: v for k, v in features_dict.items()}

    grouped_features = {k: extract_features(v, args.group_by, args.exact_match) for k, v in features_dict.items()}

    # quantify statistics and write
    depth_dict, coverage_dict = quantify_gene_coverage(
        imported_bam, grouped_features, args.feature_type, args.exact_match, args.min_depth, args.min_quality
    )
